...
- C — Live/MV: free camera/FOV/clip, MV-unit override, and separated vocal on a known-supported song/unit. Verify ordinary member selection can place the same idol in multiple positions with `allowSameIdol=true`. Then, with `Save & Replace costume changes` enabled but `Override MV unit idols` disabled, make a 5-person unit containing the same idol five times with five visibly different costumes and start the MV: all five slot-specific costumes must remain distinct. This specifically guards against the former `savedCostumes[CharaId]` fan-out bug where the last edited costume replaced every duplicate idol at stage construction. Test explicit Override-MV only after this isolation case passes. For a 5-person MV, only override **Slot 0–4**; Slot 5–7 do not participate in that MV and should be left empty.
